...
//#if defined(UNMULTIBYTE) // convert file to wide char
//	int nWideLen = 0;
//	if ( nFileByteLen )
//	{
...
MCD_STR CMarkup::UTF8ToA( MCD_CSTR pszUTF8, int* pnFailed/*=NULL*/ )
{
	...
	while ( *pUTF8 )
	{
		...
		{
#if (defined OS_ANDROID)||(defined _FO_ROUTER_) //
				cout << "wctomb in ANDROID no suppored\n";
#elif defined WINCE//
			nCharLen = pps_wctomb( szANSI, (wchar_t)nUChar );
#else
...
#if (defined OS_ANDROID)||(defined _FO_ROUTER_)
		nCharLen = 0;
		cout << "mbtowc no Implement on ANDROID\n";
#elif defined WINCE//WINCE
		nCharLen=pps_mbtowc( &wcChar, pANSI, 5 );
#else
...
#else
#ifdef WINCE //special for wince used
	wchar_t wsError[MAX_PATH];
	if(::FormatMessage(0x1200,0,::GetLastError(),0,wsError,MAX_PATH,0) > 0)
	{
		char * szTmp=PPS_WideCharToMultiByte(wsError);
		strError=szTmp;
		PPS_Free(szTmp);
	}
